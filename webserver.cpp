		...
		if (strcmp(url, "/devpost.html") == 0)
		{
			if (*up_data_size != 0)
			{
				...
				*up_data_size = 0;
				...
				if (strcmp((char *)cp->conn_arg1, "open") == 0)
				{ /* start connection */
					if (devname != NULL || usb)
					{
				...
				else if (strcmp((char *)cp->conn_arg1, "close") == 0)
				{ /* terminate */
					if (devname != NULL || usb)
				...
				else if (strcmp((char *)cp->conn_arg1, "reset") == 0)
				{ /* reset */
					Manager::Get()->ResetController(homeId);
				}
				else if (strcmp((char *)cp->conn_arg1, "sreset") == 0)
				{ /* soft reset */
					Manager::Get()->SoftReset(homeId);
				}
				else if (strcmp((char *)cp->conn_arg1, "exit") == 0)
				{ /* exit */
			...
			if (*up_data_size != 0)
			{
				...
				*up_data_size = 0;
					...
					cp->conn_arg3 != NULL && strlen((char *)cp->conn_arg3) > 0)
				{
					...
					if (strcmp((char *)cp->conn_arg1, "group") == 0)
					{ /* Group update */
						pthread_mutex_lock(&nlock);
			...
			if (*up_data_size != 0)
			{
				...
				*up_data_size = 0;
					...
					cp->conn_arg4 != NULL && strlen((char *)cp->conn_arg4) > 0)
				{
					...
					if (strcmp((char *)cp->conn_arg1, "poll") == 0)
					{ /* Poll update */
						pthread_mutex_lock(&nlock);
